...
class HeaterCooler {
    constructor(device, entity, state, platform) {
        ...
        infoService
            ...
            .setCharacteristic(Characteristic.SerialNumber, this.serial) // Serial number preserved
    ...
    updateState(state) {
        try {
            ...
            // Update target temperatures for COOL and HEAT
            if (this.state.mode === 2 && this.state.targetTemperature !== undefined) {
                this.HeaterCoolerService.getCharacteristic(Characteristic.CoolingThresholdTemperature)
                    .updateValue(this.state.targetTemperature);
            }

            if (this.state.mode === 3 && this.state.targetTemperature !== undefined) {
                this.HeaterCoolerService.getCharacteristic(Characteristic.HeatingThresholdTemperature)
                    .updateValue(this.state.targetTemperature);
            }
