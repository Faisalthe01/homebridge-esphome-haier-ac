...
SwingMode: function(swing) {
			...
			return new Promise((resolve, reject) => {
				...
				if (that.state.mode === 0) {
					that.log(`${that.name} is off. Ignoring SwingMode command.`);

					setTimeout(() => {
						const Characteristic = that.api.hap.Characteristic;
						that.HeaterCoolerService.getCharacteristic(Characteristic.SwingMode)
							.updateValue(0);
					}, 150);
        ...
        if (that.HeaterCoolerService && Characteristic) {
            ...
            if (that.state.targetTemperature !== undefined && 
                ...
                if (that.state.mode === 1) { // AUTO mode
                    ...
                    // Only apply Ã‚Â±2 logic if this is an external update (from ESPHome)
                    if (!isHomeKitUpdate) {
