            ...
            return new Promise((resolve, reject) => {
                ...
                switch (state) {
                    case 0: // AUTO
                        ...
                        // Clamp values to supported range
                ...
                setTimeout(() => {
                    ...
                    if (that.state.mode === 1) { // AUTO
                        ...
                        if (midpoint !== lastMidpointSent) {
                            const minTemp = that.config.visualMinTemperature ?? 16;
                            const maxTemp = that.config.visualMaxTemperature ?? 30;
                            uiTemps.low = Math.max(uiTemps.low ?? (midpoint - 2), minTemp);
                            uiTemps.high = Math.min(uiTemps.high ?? (midpoint + 2), maxTemp);

                            that.log(`${that.name} - [AUTO] Low:${uiTemps.low}°C High:${uiTemps.high}°C | Midpoint: ${midpoint}°C`);
                            that.state.targetTemperature = midpoint;
