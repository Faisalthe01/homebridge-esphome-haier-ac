...
const setTargetTemperature = function(that, temp, resolve, reject) {
    ...
    that.log(`${that.name} - [${that.state.mode === 2 ? 'COOL' : 'HEAT'}] HomeKit set Target Temperature: ${temp}Â°C`);
    ...
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            ...
            if (that.state.mode === 1) { // AUTO mode
                ...
                that.log(`${that.name} - [AUTO] HomeKit Set High:${temp}Â°C | Midpoint: ${midpoint}Â°C`);
                that.state.targetTemperature = midpoint;
    ...
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            ...
            if (that.state.mode === 1) { // AUTO mode
                ...
                that.log(`${that.name} - [AUTO] HomeKit Set Low:${temp}Â°C | Midpoint: ${midpoint}Â°C`);
                that.state.targetTemperature = midpoint;
                lastMidpointSent = midpoint;
            ...
            if (that.state.targetTemperature !== undefined && 
                ...
                if (that.state.mode === 1) { // AUTO mode
                    ...
                    if (!isHomeKitUpdate) {
                        ...
                        that.log(`${that.name} - [AUTO] ESPHome Update Low:${uiTemps.low}Â°C High:${uiTemps.high}Â°C | Midpoint: ${midpoint}Â°C`);
                        lastMidpointSent = midpoint;
                    }
                    isHomeKitUpdate = false;
                } 
